...
function ListStaff({ noFade }) {
    ...
    async function viewOrganization(id) {
        try {
            ...
            setShowLoadingModal(false);
            console.log("error while getting the particular data", error);
        }
    }

    async function getStaffs(currentPage, rowsPerPage, text) {
        try {
            const response = await clientService.getStaff(currentPage, rowsPerPage, text, currentUser?.companyId);
            return response
        } catch (error) {
            throw error
        }
