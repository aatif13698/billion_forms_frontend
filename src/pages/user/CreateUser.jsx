...
function CreateUser() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (client) {
                ...
                response = await clientService.updateUser({ ...dataObject, clientId: client?._id });

            } else {

                response = await clientService.createUser(dataObject);
    ...
    useEffect(() => {
        ...
        const staffMenu = administration.menu?.find(menu => menu?.name === "User");
        if (!staffMenu) return;
        setPermission([staffMenu]);
        const accessMap = {
            "/view/user": staffMenu.subMenus?.view?.access,
            "/update/user": staffMenu.subMenus?.update?.access,
            "/create/user": staffMenu.subMenus?.create?.access,
        };
    ...
    return (
        <div className="flex flex-col md:mx-4  mx-2     mt-3 min-h-screen bg-light dark:bg-dark">
            <Hamberger text={`${pathName == "/view/user" ? "User / View" : client ? "User / Update" : "User / Add New"}`} />
            <div className="w-[100%] mb-20  bg-cardBgLight dark:bg-cardBgDark shadow-lg rounded-lg p-6">
                <h2 className="md:text-2xl text-1xl font-semibold text-formHeadingLight dark:text-formHeadingDark md:mb-4 mb-2 text-start">{`${pathName == "/view/user" ? "View User" : client ? "Update User" : "Create User"}`}</h2>
                ...
                <form className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input
                            ...
                            disabled={pathName == "/view/user" ? true : false}
